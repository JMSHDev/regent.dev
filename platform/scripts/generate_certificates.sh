...
# Arguments:
#
# domain - domain of the platform (for local deployment use "localhost")
#
# Generated files:
#
...
if [ $# -ne 1 ]; then
    echo "This script requires 1 argument: domain name of the platform."
    exit 1
fi
...
openssl req -nodes -new -x509  -keyout ca.key -out ca.crt -subj "/C=UK/L=Northampton/O=regenet.dev/CN=ca.regent.dev"

# Next, we need to create a broker's key pair.
openssl genrsa -out server.key 2048

# We use the server key to create a certificate signing request.
openssl req -new -out server.csr -key server.key -subj "/C=UK/L=Northampton/O=regenet.dev/CN=$DOMAIN"
